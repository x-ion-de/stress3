  ...
  NULL
    ...
    for (b=0; b<bucket_count; b++) {
        ...
        for (i=0; i<object_count; i++) {
            snprintf(src_key, sizeof(src_key), "obj%04d", i + object_offset);
            snprintf(dest_key, sizeof(dest_key), "cop%04d", i + object_offset);

            wait_time += limiter->aquire();
            std::chrono::steady_clock::time_point begin1 = std::chrono::steady_clock::now();
            do {
                S3_copy_object(&bucketContext, src_key, bucket,
                   dest_key, 0,
